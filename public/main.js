...
function loadAnchor() {
  try {
    ...
    const anchorId = sanitizeAnchorId(parsed.anchorId || "private-anchor");
    if (anchorId === "private-anchor") return { lat: 0, lon: 0, anchorId: "private-anchor" };
    if (!isNumber(parsed.lat) || !isNumber(parsed.lon)) return { lat: 0, lon: 0, anchorId: "private-anchor" };
    return { lat: parsed.lat, lon: parsed.lon, anchorId };
  } catch (_) {
