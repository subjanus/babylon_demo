// server/index.js
// NOTE: This file is identical in behavior to your existing server,
// but uses an absolute public path + explicit / route for Render reliability.
// Express + Socket.IO server for the GPS Babylon toy.
// - Serves /public
// - Keeps authoritative world state in memory:
//     clients: socketId -> { lat, lon, yaw, color }
//     droppedBlocks: [{ id, lat, lon, color }]
//     worldOrigin: { lat, lon } set once (first valid fix)
// - Per-client counters: deleted cubes count is private to that client
// - Debug endpoints:
//     GET /debug/state
//     GET /debug/telemetry?limit=500&id=<socketId>
...
// Explicit root route (helps avoid "Cannot GET /" on some hosts)
app.get("/", (_req, res) => {
...
const clients = {};        // id -> { lat, lon, yaw, color }
...
io.on("connection", (socket) => {
  ...
  socket.on("dropCube", ({ lat, lon }) => {
    ...
    // Fallback: set origin from first drop
    if (!worldOrigin) {
      worldOrigin = { lat, lon };
      pushTelemetry({ t: Date.now(), id: socket.id, kind: "worldOriginSet", lat, lon });
    }
  ...
  // Best-effort client telemetry (does not affect game logic)
